...
<%
	...
	*/
	...
	System.out.println("[manList.jsp] currentPage(현재 페이지) : " + currentPage);
	System.out.println("[manList.jsp] rowPerPage : " + rowPerPage);
...
<body>
	...
	<form action="<%=request.getContextPath()%>/manList_practice0307.jsp">
	<select name="rowPerPage">
		<option value="none" selected disabled> ==선택== </option>
		<option value="5" <%=rowPerPage == 5?"selected":""%>> 5개씩 </option>
		<option value="10" <%=rowPerPage == 10?"selected":""%>> 10개씩 </option>
		<option value="15" <%=rowPerPage == 15?"selected":""%>> 15개씩 </option>
	</select>
	...
	<div>
		...
		%>
				<a href="<%=request.getContextPath()%>/manList_practice0307.jsp?currentPage=<%= currentPage - 1 %>&rowPerPage=<%= rowPerPage %>">[이전]</a>
		<%
			}
		%>
		...
		%>
				<a href="<%=request.getContextPath()%>/manList_practice0307.jsp?currentPage=<%= currentPage + 1 %>&rowPerPage=<%= rowPerPage %>">[다음]</a>
		<%
			}
		%>
	</div>
